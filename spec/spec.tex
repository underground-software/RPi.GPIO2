...
Reset the internal state of the object to initial state. Kills the channel's poll thread if one is running. Clears the list of callbacks. Sets line mode to \texttt{\_line\_mode\_none}.
...
An entry of type \texttt{\_PollThread} used to represent a poll thread that checks for edge events on a pin.
...
\noindent \_PollThread.\textbf{\_\_init\_\_}(channel, target, args)
        
Create a new instance of this class on \texttt{channel} that will call \texttt{target(*args)} when the poll thread is started via \texttt{line\_start\_poll}.

\textit{parameters}:
\begin{enumerate}
        \item channel --- GPIO channel
        \item target --- the callable entry point to the poll thread
...
\noindent line\_\textbf{poll\_should\_die}(channel)

Return the value \texttt{\_State.lines[channel].thread.killswitch.is\_set()}. Used by a poll thread when it considers suicide.
...
\noindent line\_\textbf{start\_poll}(channel, edge, callback, bouncetime)
...
\subsection{Data Design: Line Modes} \label{linemodes}
